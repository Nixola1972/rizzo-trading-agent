...
class AlphaTrader:
    ...
    def __init__(self, config: Optional[AlphaConfig] = None):
        ...
        logger.info(f"AlphaTrader initialized - restored {len(self.positions)} positions")
    ...
    def _load_open_positions_from_db(self):
        ...
        if not DB_AVAILABLE or not alpha_db:
            logger.warning("Database not available, cannot load positions")
            return

        try:
            open_trades = alpha_db.get_all_open_trades()
            ...
            for trade in open_trades:
                ...
                if symbol in self.positions:
                    logger.warning(f"Duplicate position for {symbol}, skipping")
                    continue

                # Convert DB row to AlphaPosition
                ...
                logger.info(f"Restored position: {trade['direction']} {symbol} @ ${position.entry_price:.2f} (open {age_min:.1f}m)")
